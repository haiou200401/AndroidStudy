...
import com.yiming.myapplication.view.IntProperty;
import com.yiming.myapplication.view.MathUtils;
 ...
 */
public class YimingFastScroller {
    ...
    private final FrameLayout mList;
    ...
    private void initHook() {
        try {
            // gloam:
            mReportScrollStateChangeMethod = AbsListView.class.getMethod("reportScrollStateChange", int.class);
        } catch (Exception exp) {
            exp.printStackTrace();;
        }
    }
    public YimingFastScroller(FrameLayout listView) {
        mList = listView;
    ...
    /**
     * Removes this YimingFastScroller overlay from the host view.
     */
    public void remove() {
    ...
    public boolean isEnabled() {
        return mEnabled && (mLongRange || mAlwaysShow);
    }
    ...
    private void onStateDependencyChanged() {
        ...
        try {
            // gloam:
            Method resolvePaddingMethod = AbsListView.class.getMethod("resolvePadding");
            resolvePaddingMethod.invoke(mList);
    ...
    public void setScrollbarPosition(int position) {
        if (position == View.SCROLLBAR_POSITION_DEFAULT) {
            ...
            try {
                // gloam:
                Method isLayoutRtlMethod = AbsListView.class.getMethod("isLayoutRtl");
                position = (Boolean)isLayoutRtlMethod.invoke(mList) ?
...
//    }
    ...
    public void updateScrollRange(int scrollRange) {
        final boolean longRange = scrollRange / mList.getHeight() >= MIN_PAGES;
        if (mLongRange != longRange) {
    ...
    private void layoutTrack() {
        ...
        final Rect container = mContainerRect;
        final int containerWidth = container.width();
        final int widthMeasureSpec = View.MeasureSpec.makeMeasureSpec(containerWidth, View.MeasureSpec.AT_MOST);
        final int heightMeasureSpec = View.MeasureSpec.makeMeasureSpec(0, View.MeasureSpec.UNSPECIFIED);
    ...
    public void onScrollChanged(int l, int t, int oldl, int oldt) {
        ...
        //final boolean hasMoreItems = totalItemCount - visibleItemCount > 0;
        final boolean hasMoreItems = true; //totalItemCount - visibleItemCount > 0;
        if (hasMoreItems && mState != STATE_DRAGGING) {
            setThumbPos(t);
        }
    ...
    private void beginDrag() {
        ...
        if (mList != null) {
            ...
            try {
                // gloam:
                //mReportScrollStateChangeMethod.invoke(mList, AbsListView.OnScrollListener.SCROLL_STATE_TOUCH_SCROLL);
            ...
            case MotionEvent.ACTION_DOWN:
                if (isPointInside(ev.getX(), ev.getY())) {
                    ...
                    try {
                        ...
                        if (!(Boolean)isInScrollingContainerMethod.invoke(mList)) {
                            beginDrag();
                            return true;
                        }
            ...
            case MotionEvent.ACTION_UP: {
                ...
                if (mState == STATE_DRAGGING) {
                    if (mList != null) {
                        ...
                        try {
                            // gloam:
                            mReportScrollStateChangeMethod.invoke(mList, AbsListView.OnScrollListener.SCROLL_STATE_IDLE);
