...
GraphicBuffer::GraphicBuffer(uint32_t width, uint32_t height, PixelFormat format, uint32_t usage):
  ...
  int32_t ctorStatus = functions.initCheck(gb);
...
int32_t GraphicBuffer::lock(uint32_t usage, void** vaddr) {
  return functions.lock(impl, usage, vaddr);
}

int32_t GraphicBuffer::unlock() {
...
int32_t GraphicBuffer::initCheck() {
