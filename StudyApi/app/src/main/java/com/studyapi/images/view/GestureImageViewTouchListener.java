...
public class GestureImageViewTouchListener implements OnTouchListener {
	...
	private GestureImageView.GestureImageViewListener imageListener;
	...
	public GestureImageViewTouchListener(final GestureImageView image, int displayWidth, int displayHeight) {
		...
		flingAnimation.setListener(new FlingAnimation.FlingAnimationListener() {
			@Override
		...
		zoomAnimation.setZoomAnimationListener(new ZoomAnimation.ZoomAnimationListener() {
			@Override
		...
		moveAnimation.setMoveAnimationListener(new MoveAnimation.MoveAnimationListener() {
				...
				if(event.getAction() == MotionEvent.ACTION_UP) {
					...
					if(event.getPointerCount() > 1) {
						...
						if(initialDistance > 0) {
							...
							initialDistance = EventDistance(event);
...
*/
	...
	/*
	360mse from other files
	 */

    public float EventDistance(MotionEvent event) {
        float x = event.getX(0) - event.getX(1);
        float y = event.getY(0) - event.getY(1);
        return (float)Math.sqrt(x * x + y * y);
    }

    public void EventMidpoint(MotionEvent event, PointF point) {
    ...
    private void midpoint(float x1, float y1, float x2, float y2, PointF point) {
        point.x = (x1 + x2) / 2.0f;
        point.y = (y1 + y2) / 2.0f;
    }
